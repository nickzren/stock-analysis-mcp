...
from stock_mcp.tools.analyze import (
    _build_analyst_summary_text,
    _build_dip_summary_text,
    _build_fundamentals_summary_text,
    _build_news_summary_text,
    _build_ownership_summary_text,
    _build_risk_summary_text,
    _build_short_interest_summary_text,
    _build_technicals_summary_text,
)
...
def test_technicals_summary_downtrend_mentions_downtrend_or_below() -> None:
    ...
    text = _build_technicals_summary_text(technicals_summary)
...
def test_technicals_summary_missing_data_does_not_crash() -> None:
    text = _build_technicals_summary_text({})
    assert isinstance(text, str)
...
def test_fundamentals_summary_unprofitable_mentions_unprofitable() -> None:
    ...
    text = _build_fundamentals_summary_text(fundamentals_summary)
    assert isinstance(text, str)
    assert "unprofitable" in text.lower()


def test_fundamentals_summary_empty_returns_none() -> None:
    assert _build_fundamentals_summary_text({}) is None
...
def test_risk_summary_extreme_mentions_risk_regime() -> None:
    ...
    text = _build_risk_summary_text(risk_summary)
    assert isinstance(text, str)
...
def test_news_summary_accepts_string_confidence() -> None:
    ...
    text = _build_news_summary_text(news_summary)
    assert isinstance(text, str)
...
def test_dip_summary_mentions_type() -> None:
    ...
    text = _build_dip_summary_text(dip_assessment)
    assert isinstance(text, str)
...
def test_ownership_short_interest_and_analyst_summaries_do_not_crash() -> None:
    ...
    ownership_text = _build_ownership_summary_text(ownership)
    assert ownership_text is None or isinstance(ownership_text, str)

    short_text = _build_short_interest_summary_text(short_interest)
    assert short_text is None or isinstance(short_text, str)

    analyst_text = _build_analyst_summary_text(analyst, current_price=195.0, currency="USD")
